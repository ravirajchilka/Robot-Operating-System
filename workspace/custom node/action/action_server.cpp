...
const float DIST_THRESHOLD = 0.1; // distance in meters which is considered close enough to the goal
...
class NavigateActionServerNode : public rclcpp::Node {
    ...
    private:
        ...
        rclcpp_action::GoalResponse handle_goal(const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const NavigateAction::Goal>goal)
        {
            (void)uuid;
        ...
        {
            std::thread {
                    std::bind(&NavigateActionServerNode::execute, this, std::placeholders::_1),
                    goal_handle
                }.detach();
        }
        ...
        void execute(const std::shared_ptr<GoalHandle> goal_handle)
        {
			...
			rclcpp::Rate loop_rate(1); // 1 second loop or 1Hz 
			feedback->distance_to_pint = DIST_THRESHOLD; // init value to start while loop

			while(feedback->distance_to_pint >= DIST_THRESHOLD) {
				feedback->distance_to_pint = std::sqrt(
			...
			std::cout << "Goal Succeded" << std::endl;
        }
